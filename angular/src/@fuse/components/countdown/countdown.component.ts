...
export class FuseCountdownComponent implements OnInit, OnDestroy {
   ...
   */
  ngOnInit(): void {
    const currDate = new Date(); // moment();
    const eventDate = new Date(this.eventDate); // moment(this.eventDate);

    // Get the difference in between the current date and event date in seconds
    let diff = Math.ceil((currDate.getTime() - eventDate.getTime()) / 1000);
